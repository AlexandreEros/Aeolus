"""Per-snapshot PE visualization: field preparation, shared color limits, and
rendering (CUDA-gated).

These exercise the numerical preparation and declarative layout WITHOUT
inspecting pixels: area-weighted anomalies, physical p_s reconstruction,
run-wide symmetric normalization, panel/axes metadata, filenames, and nonzero
PNG output on both horizontal backends.
"""
...
def test_run_color_limits_are_symmetric_and_finite(latlon_model, tmp_path):
    from planetary_sandbox.run.pe.snapshot_visualization import (
        compute_pe_run_color_limits, select_snapshot_levels)
    _run_capsule(latlon_model, tmp_path)
    levels = select_snapshot_levels(latlon_model.sigma)
    limits = compute_pe_run_color_limits(latlon_model, tmp_path, levels)
    for name in ("vorticity", "divergence", "temperature", "surface_pressure"):
        vmin, vmax = limits.symmetric_limits(name)
...
def test_run_limit_is_driven_by_the_most_extreme_snapshot(latlon_model,
                                                          tmp_path):
    """A late high-amplitude snapshot must set the shared vorticity scale."""
    from planetary_sandbox.run.pe.snapshot_visualization import (
        compute_pe_run_color_limits, select_snapshot_levels)
    from planetary_sandbox.run.pe.visualization import _synthesize
    K = latlon_model.nlev
    n = latlon_model.l_max + 1
    levels = select_snapshot_levels(latlon_model.sigma)
    coeffs = np.zeros((2, 3 * K + 1, n, n), dtype=np.complex128)
    # A modest vorticity coefficient early, a large one late (both at the
    # upper selected level), on a resolvable non-monopole mode.
    coeffs[0, levels.upper_index, 3, 1] = 1.0e-5
    coeffs[1, levels.upper_index, 3, 1] = 7.0e-4
    np.save(tmp_path / "pe_coeffs.npy", coeffs)
    np.save(tmp_path / "pe_snapshot_times.npy",
            np.asarray([0.0, 900.0], dtype=np.float64))
    limits = compute_pe_run_color_limits(latlon_model, tmp_path, levels)
    late_max = float(np.max(np.abs(
        _synthesize(latlon_model, coeffs[1, levels.upper_index],
                    subtract_mean=False))))
    assert limits.vorticity == pytest.approx(late_max, rel=1e-6)
...
def test_snapshot_figure_layout_and_titles(latlon_model, tmp_path):
    ...
    from planetary_sandbox.run.pe.snapshot_visualization import (
        build_pe_snapshot_figure, compute_pe_run_color_limits,
        prepare_pe_snapshot_fields, select_snapshot_levels)
    ...
    _, spec = build_pe_snapshot_figure(
        latlon_model, fields, limits, scenario="thermal_wave",
        backend_label="lat-lon (Gauss)", run_id="run-xyz")
    ...
    # Upper and lower vorticity panels share one frozen symmetric scale.
    vort = [panel for panel in map_panels
            if panel.title.lower().startswith("relative vorticity")]
    assert len(vort) == 2
    limits_seen = {(p.normalization.vmin, p.normalization.vmax) for p in vort}
...
def test_surface_pressure_panel_spans_both_map_rows(latlon_model, tmp_path):
    ...
    limits = compute_pe_run_color_limits(latlon_model, tmp_path, levels)
    fields = prepare_pe_snapshot_fields(
        latlon_model, coeffs[-1], index=0, total=len(times),
        time_seconds=float(times[0]), levels=levels)
    _, spec = build_pe_snapshot_figure(latlon_model, fields, limits)
...
# ---------------------------------------------------------------------------
# Rendering to a run capsule
# ---------------------------------------------------------------------------

def test_render_all_writes_one_png_per_snapshot(latlon_model, tmp_path):
    from planetary_sandbox.run.pe.snapshot_visualization import (
        PE_SNAPSHOTS_DIRNAME, render_all_pe_snapshots)
    times = _run_capsule(latlon_model, tmp_path, n_snapshots=3)
    paths = render_all_pe_snapshots(latlon_model, tmp_path,
                                    scenario="thermal_wave")
    assert len(paths) == len(times)
    snap_dir = tmp_path / "figures" / PE_SNAPSHOTS_DIRNAME
    pngs = sorted(snap_dir.glob("snapshot_*.png"))
    assert len(pngs) == len(times)
    assert [p.name for p in pngs] == [f"snapshot_{i:04d}.png"
                                      for i in range(len(times))]
    for png in pngs:
        assert png.stat().st_size > 0


def test_render_all_preserves_stored_data_and_summary(latlon_model, tmp_path):
    from planetary_sandbox.run.pe.snapshot_visualization import (
    ...
    render_all_pe_snapshots(latlon_model, tmp_path, scenario="thermal_wave")
...
def test_render_all_on_geodesic_and_exact_rest(tmp_path):
    from planetary_sandbox.run.pe.snapshot_visualization import (
        PE_SNAPSHOTS_DIRNAME, render_all_pe_snapshots)
    model = _make_model(grid_type="geodesic", l_max=10)
    times = _run_capsule(model, tmp_path, scenario="isothermal_rest",
                         n_snapshots=2)
    paths = render_all_pe_snapshots(model, tmp_path,
                                    scenario="isothermal_rest")
    assert len(paths) == len(times)
    snap_dir = tmp_path / "figures" / PE_SNAPSHOTS_DIRNAME
    assert len(sorted(snap_dir.glob("snapshot_*.png"))) == len(times)
