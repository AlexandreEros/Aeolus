...
the `feat/shallow-water` milestone, the **inviscid rotating shallow-water
equations with optional fixed analytic bottom topography**
([SHALLOW_WATER.md](SHALLOW_WATER.md)). It does **not** solve the primitive
equations or any general-circulation / weather model, and it has no vertical
structure, time-dependent or data-driven terrain, forcing, moisture, or
thermodynamics. The separate terrain generated by `Planet.generate` remains
decorative; only the SWE core's band-limited `Topography` enters dynamics.
