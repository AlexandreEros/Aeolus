...
class VorticityViewer:
    def __init__(self, 
        ...
        assert vorticity_snapshots.ndim == 3 or vorticity_snapshots.size == 0, "Snapshots must be a 3D array (time, lat, lon) or empty."
        ...
        expected_nlon = self.planet.sh.num_lon
        if self.zeta_init.shape[1] == expected_nlon + 1:
        ...
        bve = BarotropicVorticity(planet)
    ...
    def plot_all_snapshots(self, scenario="snapshots", out_dir=None):
        ...
        # Create figure with nsnap * 3 subplots
        fig, axes = plt.subplots(nsnap, 3, figsize=(18, 6 * nsnap))
        ...
        for idx, zeta_snap in enumerate(self.snapshots):
            ...
            # Vorticity
            im0 = axes[idx, 0].imshow(np.flip(zeta_snap, axis=0),
                                 extent=(0, 360, -90, 90),
                                 cmap='RdBu_r',
                                 aspect='equal',
                                 origin='lower')
            ...
            # Streamfunction
            im1 = axes[idx, 1].imshow(np.flip(psi_grid, axis=0),
                                 extent=(0, 360, -90, 90),
                                 cmap='viridis',
                                 aspect='equal',
                                 origin='lower')
            ...
            axes[idx, 2] = plot_velocity_streamlines((u, v), self.planet, ax=axes[idx, 2],
                                                      title=f"Flow @ t={time_hrs:.1f}h")
    ...
    def plot_summary(self):
        ...
        im1 = ax1.imshow(np.flip(zeta_init, axis=0),
                         extent=(0, 360, -90, 90),
                         cmap='RdBu_r',
                         aspect='equal',
                         origin='lower')
        ...
        im2 = ax2.imshow(np.flip(zeta_final, axis=0),
                         extent=(0, 360, -90, 90),
                         cmap='RdBu_r',
                         aspect='equal',
                         origin='lower')
        ...
        plot_velocity_streamlines(self.vel_init, self.planet, ax=ax3, title="Initial Flow")

        # --- 4. Final Flow (Middle Right) ---
        ax4 = fig.add_subplot(gs[1, 2])
        plot_velocity_streamlines(self.vel_final, self.planet, ax=ax4, title="Final Flow")

        # --- 5. Initial Streamfunction (Bottom Left) ---
        ax5 = fig.add_subplot(gs[2, 0])
        im5 = ax5.imshow(np.flip(psi_init, axis=0),
                         extent=(0, 360, -90, 90),
                         cmap='viridis',
                         aspect='equal',
                         origin='lower')
        ...
        im6 = ax6.imshow(np.flip(psi_final, axis=0),
                         extent=(0, 360, -90, 90),
                         cmap='viridis',
                         aspect='equal',
                         origin='lower')
        ...
        n_lat, n_lon = zeta_init.shape
        if self.planet.grid.lat_grid.shape == zeta_init.shape:
            lats = self.planet.grid.lat_grid
        ...
        if self.snapshots is not None and self.times is not None:
            ...
            enstrophy = np.sqrt(np.mean(self.snapshots**2, axis=(1, 2)))
